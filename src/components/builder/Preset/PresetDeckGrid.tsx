...
const PresetDeckGrid = ({ deck, deckSize, personality }: PresetDeckGridProps) => {
  ...
  const Slot = ({ apostle }: { apostle: Apostle | null }) => (
    <div
      className={`aspect-square rounded-lg border-2 transition-all ${
        !apostle
          ? 'border-base-300 bg-base-100 border-dashed'
          : `group relative overflow-hidden shadow-sm hover:shadow-md ${getPersonalityBackgroundClass(personality as Personality)}`
      }`}
    >
      {apostle && (
        <>
  ...
  const Column = ({
    ...
    <div className="flex flex-col gap-0.5">
      <div className={`${colorClass} rounded-t-lg py-1.5 text-center text-xs font-bold sm:text-sm`}>
        {title}
      </div>
      <div className="flex flex-col gap-0.5">
        {items.map((item, idx) => (
          <Slot key={`${title}-${item?.id || idx}`} apostle={item} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="grid w-full max-w-sm grid-cols-3 gap-2">
      <Column title="후열" items={backLine} colorClass="bg-primary text-primary-content" />
      <Column title="중열" items={midLine} colorClass="bg-success text-success-content" />
      <Column title="전열" items={frontLine} colorClass="bg-error text-error-content" />
    </div>
  );
};
